...
int decryption(unsigned char* key, unsigned char* iv, ifstream& in_file, size_t in_len, ofstream& out_file) {
	...
	for (size_t i = 0; i < in_len - 64; i += block) {
		len = (((in_len - 64) - i) >= block ? block : (in_len - 64) - i);
		in_file.read((char*)input, len);
	...
	bool correct = 1;
	for (int i = 0; i < 64; ++i) {
		cout << given_hash[i] << " " << hash_output[i];// << endl;
...
int encryption2(unsigned char* key, unsigned char* iv, FILE* in_file, FILE* out_file) {
	mbedtls_cipher_context_t crypt_ctx;
	mbedtls_sha512_context hash_ctx;
	const size_t block = 16;
	size_t out_block = 16;
	...
	for (size_t i = 0; i < in_len; i += block) {
		size_t len = ((in_len - i) > block ? block : (in_len - i));
		fread(input, sizeof(unsigned char), len, in_file);
		//in_file.read((char*)input, len);
		mbedtls_cipher_update(&crypt_ctx, input, len, output, &out_block);
		mbedtls_sha512_update(&hash_ctx, output, out_block);
	...
	//out_file.write((char*)output, out_block);
	mbedtls_sha512_finish(&hash_ctx, hash_output);
	fwrite(hash_output, sizeof(unsigned char), 64, out_file);
	//out_file.write((char*)hash_output, 64);

	return 0;
}

int decryption2(unsigned char* key, unsigned char* iv, FILE* in_file, FILE* out_file) {
	mbedtls_cipher_context_t crypt_ctx;
	mbedtls_sha512_context hash_ctx;
	//--in_len;
	...
	mbedtls_sha512_finish(&hash_ctx, hash_output);
	fread(given_hash, sizeof(unsigned char), 64, in_file);
	//in_file.read((char*)given_hash, 64);
	//cout << in_file.gcount() << endl;
	//cout.write((char*)given_hash, 64);
	//cout << endl;
